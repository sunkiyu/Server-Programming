...
void ServerPacketHandler::Handle_C_CHAT(BYTE* buffer, int32 len,SessionRef curSession)
{
	...
	CHAR* msg = reinterpret_cast<CHAR*>(buffer + sizeof(PKT_S_CHAT));
	cout << "user"<<curSession->GetSessionId() << " : " << msg << endl;
	...
	memcpy(_pkt, pChat, sizeof(PKT_S_CHAT));
	BYTE* str = _bw.Reserve<BYTE>(1024);
...
void Session::Disconnect(const WCHAR* cause)
{
	...
	// TEMP
	wcout <<"user"<<GetSessionId()<<" Disconnect : " << cause << endl;
